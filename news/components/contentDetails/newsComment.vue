                ...
                <li class="item">
                    <dl>
                        ...
                        <dd>
                            ...
                            <p class="ct " :class="[arr[index] ? 'ellipsis0' : 'ellipsis2']">{{item.content}}</p>
                            <p class="time">
                                <span><span v-if="arrShow[index]" style="color:#0060D0;" @click="isSHow(index)">{{arr[index] ? "收起" :'展开'}}<b style="color:#fff;">多</b></span>{{item.createTime.substr(0,16)}}</span>
...
export default {
  name: "newsComment",
  props: ["newsCommentList", "remove",'isdelComment'],
  data() {
    return {
      isDel: false,
      imgSrc,
